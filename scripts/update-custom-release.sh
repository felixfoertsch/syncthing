...
rebuild_existing="${CUSTOM_RELEASE_REBUILD_EXISTING:-0}"
...
tag_exists() {
	...
	local status
	if git ls-remote --quiet --exit-code --refs "$push_remote" "refs/tags/$tag" >/dev/null; then
		return 0
	else
		status=$?
	fi
	# ls-remote returns 2 for a missing ref; transport/auth failures are not absence.
	[[ "$status" == "2" ]] || die "could not check tag $tag on $push_remote"
	return 1
}
...
checkout_existing_release() {
	local custom_tag="$1"
	local branch="$2"
	# Fetch without --force: never replace an existing tag with a different object.
	if ! git rev-parse --quiet --verify "refs/tags/$custom_tag" >/dev/null; then
...
test_release() {
	...
	rebuild_assets_once
	go test ./lib/api/auto ./lib/fs ./lib/ignore ./lib/scanner ./lib/model ./lib/watchaggregator
	# These tests exercise the fork's behavior, not just upstream compatibility.
...
- Remote .stignore creation, edits and deletion automatically trigger a full rescan.
- If ignore patterns match .stignore (for example .* or *), put !/.stignore first.
- All peers syncing .stignore must run this fork. Shared rules are not a privacy boundary.
...
main() {
	...
	if tag_exists "$custom_tag" && [[ "$force" != "1" ]]; then
		if [[ "$rebuild_existing" != "1" ]]; then
			log "Custom tag $custom_tag already exists; nothing to do."
			return
		fi
		checkout_existing_release "$custom_tag" "$branch"
	else
...
# Allow the workflow preflight and regression tests to reuse tag resolution.
